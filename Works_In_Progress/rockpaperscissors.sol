...
contract RockPaperScissors {
  ...
  function getOutcome() public revealPhaseEnded returns (Outcomes) {
        ...
        pay(addrA, addrB, addrC, betPlayerA, outcome, HOUSE_PCT);

        return outcome;
    }

    // Pay the winner(s).
    function pay(address payable addrA, address payable addrB, address payable addrC, uint betPlayerA, Outcomes outcome, uint house_fee) private {
        // Uncomment lines below if you need to adjust the gas limit

        uint vig = ((address(this).balance) / 100) * house_fee;
