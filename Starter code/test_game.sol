    ...
    function getOutcome() public has_player_played returns (uint) {
        ...
        if (randomNum % 2 == 0) {
            ...
            if (playerChoice == 2) {
                ...
                paid = 1;            
                payable(playerA).transfer(address(this).balance);  //changed this to be compatible with latest solidity compiler
        ...
        if (paid == 0) {
            ...
            //reset();  // Reset game before paying to avoid reentrancy attacks
            paid = 1;
			payable(HouseWallet).transfer(address(this).balance);
